...
describe('Criação completa de um contrato para ser recusado pela rulebook no decision', () => {
    ...
    it('A proposta deve ser recusada pelo motivo de renda mensal insuficiente.', () => {
      ...
      cy.get('[id*="Input_BirthDate"]').type('2006-05-20', { force: true });
...
cy.get('[id*="AddressDateInput"]').type('2022-03-25', { force: true });
//Dados profissinais
...
//Rendimentos
cy.get('[id*="MonthlyLiquidSalaryInput"]').clear().type("111");
cy.get('[id*="IncomeNotProvenInput"]').clear().type("10");
...
cy.get('[id*="RadioButton1-input"]').eq(0).click();
 ...
 .filter(':contains("RECUSADA")')
 .should('have.text', 'RECUSADA');
 cy.wait(5000);
    });
}); 